...
private:
  ...
  const int NUMBER_OF_BOXES =
      100; // Number of containers along the x and y axis

  const int FILTER_TRESHOLD = 30; // Weight of a box blow which it is filtered

  const int HOUGH_TRESHOLD =
      30; // Weight above which a point in the hough table is considered

  static unsigned const int RHO_RESOLUTION = 500; // Radius
  static unsigned const int THETA_RESOLUTION =
      359; // Angle (360 degrees with 0 = 360, thus 359)
  ...
  class GridBox {
  public:
    GridBox(double upperYLimit, double lowerYLimit, double upperXLimit,
            double lowerXLimit)
        : m_upperYLimit(upperYLimit), m_lowerYLimit(lowerYLimit),
          m_upperXLimit(upperXLimit), m_lowerXLimit(lowerXLimit) {
  ...
  // std::array<std::array<int, RHO_RESOLUTION>, THETA_RESOLUTION>
  // m_accumulatorTable;

  // Results container

  // The gridMaker divides the x-y plane into a grid of a parameterizable
  // resolution where the value of a given box is determined by the number of
  // reflections contained within it's borders.
  void gridMaker();

  // The gridWeightSetter iterates across all the boxes in the grid for all
  // Pulses in the Scan container and sets the weight of the box based on the
  // number of points inside the border of the box.
  void gridWeightSetter();

  // The gridFilter iterates across all the boxes in the grid and filters the
  // ones whose weight falls below some arbitrary treshold.
  void gridFilter();

  // The hough function implments the hough transform, which acts on the
  // elements of the grid similar to the way it would act on the pixels of an
  // image.
  void hough();

  // The accumulatorNormalizer function tansforms the rho value, with a range of
  // [(-sqrt(2) * MAX_Y) to (sqrt(2) * MAX_Y)] into a range of [0-1000]. In this
  // respect it is some type of normalization. This is a helper functon for
  // hough().
  ...
  // The filterTopIntersects function finds the peek points in the accumulator
  // grid. This is a helper function for the hough() function.
