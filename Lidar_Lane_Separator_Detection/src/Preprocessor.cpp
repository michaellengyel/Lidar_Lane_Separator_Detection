...
void Preprocessor::loadScan(Scan &scan, IOHandler &ioHandler) {
  ...
  const unsigned int numberOfcolumns =
      static_cast<unsigned int>(dataFormatA::NUMBER_OF_COLUMNS);
...
void Preprocessor::filterData(Scan &scan, std::string blocks[]) {
  ...
  // Filtering logic
  bool distanceInRange = (std::stod(blocks[m_columnDistance]) <= maxDistance) &&
                         (std::stod(blocks[m_columnDistance]) > minDistance);
  bool intensityInRange =
      (std::stod(blocks[m_columnIntensity]) <= maxIntensity) &&
      (std::stod(blocks[m_columnIntensity]) > minIntensity);

  bool xInRange = (std::stod(blocks[m_columnX]) <= m_maxX) &&
                  (std::stod(blocks[m_columnX]) > m_minX);
  bool yInRange = (std::stod(blocks[m_columnY]) <= m_maxY) &&
                  (std::stod(blocks[m_columnY]) > m_minY);
  bool zInRange = (std::stod(blocks[m_columnZ]) <= m_maxZ) &&
                  (std::stod(blocks[m_columnZ]) > m_minZ);
  ...
  if ((distanceInRange && intensityInRange) &&
      (readingDuplicate && orthoInRange)) {

    Point point(std::stod(blocks[m_columnX]), std::stod(blocks[m_columnY]),
                std::stod(blocks[m_columnZ]));
